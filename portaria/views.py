...
class MailProc:
    MAX_COMMON_SEND_A_LOOP = 40
    MAX_SPECIAL_SEND_A_LOOP = 5
    MAILS_TRY_AGAIN_WAIT_TIME = 5
    MAILS_UNKNOWN_ERROR_MAX_CONTINUE_TIME = 20
    ENCODE = "gb18030"
    ...
    def get_queryset(self):
        ...
        if autor.is_staff:
            qs = (
                ...
                .filter(
                    hr_chegada__month=datetime.datetime.now().month,
                    hr_chegada__year=datetime.datetime.now().year,
                )
                .order_by("-hr_chegada")
            )
        else:
        ...
        try:
            ...
            if form_input1 and form_input2:
                self.dateparse1 = datetime.datetime.strptime(
                    form_input1, "%Y-%m-%d"
                ).replace(hour=00, minute=00)
                self.dateparse2 = datetime.datetime.strptime(
                    form_input2, "%Y-%m-%d"
                ...
                if autor.is_staff:
                    qs = (
                        Cadastro.objects.all()
                        .filter(
                            hr_chegada__gte=self.dateparse1,
                            hr_chegada__lte=self.dateparse2,
                        )
                        .order_by("-hr_chegada")
                    )
                else:
...
def cadastroentrada(request):
    if request.user.is_authenticated:
        ...
        return render(
            request,
            "portaria/portaria/cadastroentrada.html",
            {"cadastro": cadastro, "form": form},
        )
    else:
...
def cadastrosaida(request):
    if request.user.is_authenticated:
            ...
            if form.is_valid():
                ...
                loc_placa = (
                    Cadastro.objects.filter(
                        placa=s_query, hr_chegada__gte=days_back, hr_saida=None
                    )
                    .order_by("-hr_chegada")
                    .first()
                )
                try:
                    ...
                    messages.error(request, "Não encontrado")
                    return render(
                        request, "portaria/portaria/cadastrosaida.html", {"form": form}
                    )
                else:
                    Cadastro.objects.filter(pk=loc_placa.id).update(
                        hr_saida=timezone.now(),
                        destino=q_query,
                        kilometragem=km,
                        autor=request.user,
                    )
        ...
        return render(
            request,
            "portaria/portaria/cadastrosaida.html",
            {"auth_message": auth_message},
        )
...
def cadastro(request):
    if request.user.is_authenticated:
        ...
        auth_message = "Usuário não autenticado, por favor logue novamente"
        return render(
            request, "portaria/portaria/cadastro.html", {"auth_message": auth_message}
        )
...
def cadpaletes(request):
    ...
    if request.method == "POST":
        ...
        if qnt and fil and emp and tp_p:
            ...
            try:
                ...
                messages.error(
                    request, "Por favor digite um valor numérico para quantidade"
                )
                return redirect("portaria:cadpaletes")
            else:
                cliente = Cliente.objects.filter(razao_social_motorista=emp).first()
                ClienteFiliais.objects.filter(cliente=cliente, filial=fil).update(
                    saldo=(F("saldo") + int(qnt))
                )

                for x in range(0, int(qnt)):
                    PaleteControl.objects.create(
                        loc_atual=fil.sigla, tp_palete=tp_p, autor=request.user
                    )
                    if x == 2000:
                        break
                messages.success(
                    request, f"{qnt} Paletes foram cadastrados com sucesso"
                )
    ...
    return render(
        request,
        "portaria/palete/cadpaletes.html",
        {"filiais": filiais, "tp_emp": tp_emp},
    )
...
def paletecliente(request):
    ...
    if filial_selecionada or cliente_selecionado:
        if filial_selecionada:
            form = ClienteFiliais.objects.filter(
                filial__id=filial_selecionada
            ).order_by("cliente__razao_social_motorista")
        elif cliente_selecionado:
            form = ClienteFiliais.objects.filter(
                cliente__razao_social_motorista__contains=cliente_selecionado
            ).order_by("cliente__razao_social_motorista")
        ...
        return render(
            request,
            "portaria/palete/paletecliente.html",
            {"form": form, "tcount": tcount, "filiais": filiais, "clientes": clientes},
        )

    return render(
        request,
        "portaria/palete/paletecliente.html",
        {"filiais": filiais, "clientes": clientes},
    )
        ...
        if nome and tipo_cad:
            try:
                ...
                for filial in filiais:
                    ClienteFiliais.objects.create(
                        cliente=cliente, filial=filial, saldo=0
                    )
...
def saidapalete(request):
    ...
    if request.method == "POST":
        ...
        if qnt and fil and emp and tp_p:
            ...
            ClienteFiliais.objects.filter(cliente=cli, filial=fili).update(
                saldo=(F("saldo") - qnt)
            )
            for q in range(0, qnt):
                PaleteControl.objects.filter(
                    loc_atual=fili.sigla, tp_palete=tp_p
                ).first().delete()
            messages.success(request, "Saidas cadastradas com sucesso")
            return redirect("portaria:paletecliente")
    return render(
        request,
        "portaria/palete/saidapalete.html",
        {"filiais": filiais, "clientes": clientes},
    )
...
def frota(request):
    ...
    if request.method == "GET":
        ...
        if pla and mot != "Selecione...":
            try:
                ...
                return redirect(
                    "portaria:checklistfrota", placa_id=pla1, moto_id=mot1.codigomot
                )
        elif pla and mot == "Selecione...":
            messages.error(request, "Insira o motorista")
            return render(
                request,
                "portaria/frota/frota.html",
                {"form": form, "motos": motoristas},
            )
    ...
    return render(
        request, "portaria/frota/frota.html", {"form": form, "motos": motoristas}
    )
...
def checklistfrota(request, placa_id, moto_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            pla.save()
            messages.success(
                request, f"Checklist para placa {pla} e motorista {mot} concluído!"
            )
            return HttpResponseRedirect(reverse("portaria:frota"))
    return render(
        request,
        "portaria/frota/checklistfrota.html",
        {"form": form, "pla": pla, "mot": mot},
    )
    ...
    if func:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.error(
                    request, "Algo deu errado, por favor contate seu administrador."
                )
                return redirect("portaria:index")
    return render(
        request,
        "portaria/pj/atualizarfunc.html",
        {"fields": form, "allfuncs": allfuncs, "func": func},
    )


def get_funcionarios_clt(request):
    conn = conndb()
    cur = conn.cursor()
...
SELECT DISTINCT
       ...
       END filial,
       FL.NOMEFUNC nome,
       FL.DTNASCTOFUNC data_nascimento,
       FL.DTADMFUNC data_admissao,
       ...
       FL.ENDRUAFUNC rua,
       FL.ENDNRFUNC numero,
       FL.ENDCOMPLFUNC complemento,
       FL.ENDCEPFUNC cep,
       FL.ENDBAIRROFUNC bairro,
       FL.ENDCIDADEFUNC cidade,
       FL.CODIGOUF uf,
       FL.CODBANCO banco,
       FL.CODAGENCIA agencia,
       FL.CONTACORFUNC conta
FROM
       FLP_FUNCIONARIOS FL
WHERE
       FL.SITUACAOFUNC = 'A' AND
...
ORDER BY 
    ...
    for funcionario in data:
        filial = Filiais.objects.get(id=funcionario["filial"])
        funcionario["filial"] = filial

        try:
            if not funcionario["rg"]:
                funcionario["rg"] = 1

            if not funcionario["banco"]:
                funcionario["banco"] = 1

            if not funcionario["agencia"]:
                funcionario["agencia"] = 1

            if not funcionario["conta"]:
                funcionario["conta"] = 1

            Funcionarios.objects.get(cpf=funcionario["cpf"])
...
def get_documento_funcionario_clt(request, id: int):
    func = Funcionarios.objects.get(pk=id)

    data = {
        "NOME": upper(func.nome),
        "RG": func.rg,
        "CPF": func.cpf,
        "DATA NASCIMENTO": datetime.datetime.strftime(func.data_nascimento, "%d/%m/%Y"),
        "EMPRESA": upper(func.empresa),
        "TIPO CONTRATO": func.tipo_contrato,
        "FILIAL": upper(func.filial.sigla),
        "DATA ADMISSÃO": datetime.datetime.strftime(func.data_admissao, "%d/%m/%Y"),
        "RUA": upper(func.rua),
        "NUMERO": func.numero,
        "COMPLEMENTO": upper(func.complemento) if func.complemento else "NÃO INFORMADO",
        "CEP": func.cep,
        "BAIRRO": upper(func.bairro),
        "CIDADE": upper(func.cidade),
        "UF": upper(func.uf),
        "BANCO": func.banco,
        "AGENCIA": func.agencia,
        "CONTA": func.conta,
        "PIX": upper(func.pix) if func.pix else "NÃO INFORMADO",
    }

    if func.epi:
        data["CELULAR"] = "SIM" if func.epi.celular_modelo else "NÃO"
        data["NOTEBOOK"] = "SIM" if func.epi.notebook_modelo else "NÃO"
    else:
        data["CELULAR"] = "NÃO"
        data["NOTEBOOK"] = "NÃO"

    return gerar_ficha(data, func)


def get_documento_funcionario_pj(request, id: int):
    func = FuncPj.objects.get(pk=id)
    ...
    data = {
        ...
        "CARGO": func.cargo,
        "TIPO CONTRATO": func.tipo_contrato,
        "EMPRESA": upper(func.filial.empresa),
        "FILIAL": upper(func.filial.sigla),
        "DATA ADMISSÃO": datetime.datetime.strftime(func.admissao, "%d/%m/%Y"),
        "BANCO": func.banco,
        "AGENCIA": func.ag,
        "CONTA": func.conta,
        "PIX": upper(func.pix) if func.pix else "NÃO INFORMADO",
...
def gerar_ficha(data, funcionario):
    try:
        pdf = FPDF(orientation="P", unit="mm", format=(210, 297))
        pdf.add_page()
        pdf.ln()
        ...
        pdf.output(f"FIXA CADASTRAL - {upper(funcionario.nome)}.pdf")
        with open(f"FIXA CADASTRAL - {upper(funcionario.nome)}.pdf", "rb") as f:
            response = HttpResponse(f.read(), content_type="application/pdf")
        f.close()
        os.remove(f"FIXA CADASTRAL - {upper(funcionario.nome)}.pdf")
        response[
            "Content-Disposition"
        ] = f"filename=FIXA CADASTRAL - {upper(funcionario.nome)}.pdf"
        return response
...
def ficha_cadastral(request):
    ...
    if request.method == "GET":
        ...
        if nome or filial or cpf or tipo_contrato:
            filtro_clt = {"ativo": True}
            filtro_pj = {"ativo": True}

            if not tipo_contrato or tipo_contrato == "CLT":
                if not nome is None:
                    filtro_clt["nome__contains"] = nome
                if not filial is None:
                    filtro_clt["filial__id"] = filial
                if not cpf is None:
                    filtro_clt["cpf__contains"] = cpf

                funcionarios_clt = Funcionarios.objects.filter(**filtro_clt).order_by(
                    "nome"
                )

                funcionarios += list(funcionarios_clt)

            if not tipo_contrato or tipo_contrato == "PJ":
                if not nome is None:
                    filtro_pj["nome__contains"] = nome
                if not filial is None:
                    filtro_pj["filial__id"] = filial
                if not cpf is None:
                    filtro_pj["cpf_cnpj__contains"] = cpf

                funcionarios_pj = FuncPj.objects.filter(**filtro_pj).order_by("nome")

                funcionarios += list(funcionarios_pj)

            return render(
                request,
                "portaria/pj/ficha_cadastral.html",
                {"funcionarios": funcionarios, "filiais": filiais},
            )

    return render(request, "portaria/pj/ficha_cadastral.html", {"filiais": filiais})


def cadastrar_epi_funcionario_clt(request, id: int):
    if request.method == "POST":
        celular_modelo = upper(request.POST.get("celular_modelo"))
        celular_numero_ativo = request.POST.get("celular_numero_ativo")
        notebook_modelo = upper(request.POST.get("notebook_modelo"))
        notebook_numero_ativo = request.POST.get("notebook_numero_ativo")
        observacao = upper(request.POST.get("observacao"))

        try:
            funcionario = Funcionarios.objects.get(id=id)

            if funcionario.epi:
                epi = FuncionariosEPIs.objects.get(funcionario_clt__id=funcionario.id)

                epi.celular_modelo = celular_modelo if celular_modelo else None
                epi.celular_numero_ativo = (
                    celular_numero_ativo if celular_numero_ativo else None
                )
                epi.notebook_modelo = notebook_modelo if notebook_modelo else None
                epi.notebook_numero_ativo = (
                    notebook_numero_ativo if notebook_numero_ativo else None
                )
                epi.observacao = observacao if observacao else None

                epi.save()

                messages.success(request, "EPI Editado")
            else:
                raise ValueError()

        except ValueError as v:
            data = {
                "celular_modelo": celular_modelo if celular_modelo else None,
                "celular_numero_ativo": celular_numero_ativo
                if celular_numero_ativo
                else None,
                "notebook_modelo": notebook_modelo if notebook_modelo else None,
                "notebook_numero_ativo": notebook_numero_ativo
                if notebook_numero_ativo
                else None,
                "observacao": observacao if observacao else None,
            }
            epi = FuncionariosEPIs.objects.create(**data)
...
def cadastrar_epi_funcionario_pj(request, id: int):
    if request.method == "POST":
        ...
        try:
            funcionario = FuncPj.objects.get(id=id)

            if funcionario.epi:
                epi = FuncionariosEPIs.objects.get(funcionario_pj__id=funcionario.id)
...
def inserirmesmovalores(request, id):
    if id:
        try:
            ...
            nf = (
                NfServicoPj.objects.filter(funcionario=funcionario)
                .order_by("-id")
                .first()
            )
        ...
        if len(nf) > 0:
            query = (
                ...
                .annotate(
                    total=(
                        ...
                        - (F("adiantamento") + F("desc_convenio") + F("outros_desc"))
                    )
                )
            )
        else:
            query = (
                ...
                .annotate(
                    faculdade=Coalesce(
                        Sum(
                            "nfservicopj__faculdade",
                            filter=Q(
                                nfservicopj__data_emissao__month=datetime.datetime.now().month,
                                nfservicopj__data_emissao__year=datetime.datetime.now().year,
                            ),
                        ),
                        Value(0.0),
                    ),
                    cred_convenio=Coalesce(
                        Sum(
                            "nfservicopj__cred_convenio",
                            filter=Q(
                                nfservicopj__data_emissao__month=datetime.datetime.now().month,
                                nfservicopj__data_emissao__year=datetime.datetime.now().year,
                            ),
                    ...
                    outros_cred=Coalesce(
                        Sum(
                            "nfservicopj__outros_cred",
                            filter=Q(
                                nfservicopj__data_emissao__month=datetime.datetime.now().month,
                                nfservicopj__data_emissao__year=datetime.datetime.now().year,
                            ),
                    ...
                    aux_moradia=Coalesce(
                        Sum(
                            "nfservicopj__aux_moradia",
                            filter=Q(
                                nfservicopj__data_emissao__month=datetime.datetime.now().month,
                                nfservicopj__data_emissao__year=datetime.datetime.now().year,
                            ),
                    ...
                    desc_convenio=Coalesce(
                        Sum(
                            "nfservicopj__desc_convenio",
                            filter=Q(
                                nfservicopj__data_emissao__month=datetime.datetime.now().month,
                                nfservicopj__data_emissao__year=datetime.datetime.now().year,
                            ),
                    ...
                    outros_desc=Coalesce(
                        Sum(
                            "nfservicopj__outros_desc",
                            filter=Q(
                                nfservicopj__data_emissao__month=datetime.datetime.now().month,
                                nfservicopj__data_emissao__year=datetime.datetime.now().year,
                            ),
...
def cadservicospj(request, args):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            calc.save()
            messages.success(
                request, f"Valores cadastrados com sucesso para {calc.funcionario}"
            )
    ...
    return render(
        request, "portaria/pj/cadservicospj.html", {"form": form, "func": func}
    )


def decimopj(request):
    allfunc = (
        FuncPj.objects.filter(ativo=True)
        .annotate(parc1=F("pj13__pgto_parc_1"), parc2=F("pj13__pgto_parc_2"))
        .order_by("nome")
    )
    func = request.GET.get("srcfunc")
    if func:
        allfunc = (
            FuncPj.objects.filter(nome__icontains=func, ativo=True)
            .annotate(parc1=F("pj13__pgto_parc_1"), parc2=F("pj13__pgto_parc_2"))
            .order_by("nome")
        )
...
def caddecimo2(request, idfunc):
    ...
    if pj13.objects.filter(funcionario=func, pgto_parc_2=None).exists():
        ...
        if parc:
            pj13.objects.filter(funcionario=func).update(
                pgto_parc_2=timezone.now(), autor=autor
            )
            messages.success(request, "Cadastro segunda parcela feito com sucesso")
            return redirect("portaria:decimopj")
        return render(
            request, "portaria/pj/caddecimo2.html", {"func": func, "form": form}
        )
...
def decimoview(request):
    ...
    if period:
        if period == "pgto_parcela_1":
            array = []
            allfuncs = FuncPj.objects.filter(
                ativo=True,
                pj13__pgto_parc_1__isnull=False,
                pj13__pgto_parc_2__isnull=True,
            )
            for q in allfuncs:
            ...
            return render(
                request,
                "portaria/pj/decimoview.html",
                {"allfuncs": allfuncs, "array": array},
            )
        elif period == "pgto_parcela_2":
            array = []
            allfuncs = FuncPj.objects.filter(
                ativo=True, pj13__pgto_parc_2__isnull=False
            )
            for q in allfuncs:
                query = FuncPj.objects.filter(pk=q.id).annotate(
                    valor=F("pj13__valor"),
...
def bonusviewpj(request):
    ...
    return render(
        request, "portaria/pj/bonusviewpj.html", {"bonus": bonus, "mostrar": mostrar}
    )
...
def bonusquit(request, idbpj):
    ...
    try:
        BonusPJ.objects.filter(pk=bonus.id).update(
            quitado=True, data_quitacao=timezone.now()
        )
    except ObjectDoesNotExist:
        ...
        messages.success(
            request, f"Férias quitadas para o funcionário {bonus.funcionario.nome}"
        )
        return redirect("portaria:bonusviewpj")
...
def get_contrato_csv(request):
    ...
    try:
        ...
        response = HttpResponse(
            ...
            headers={
                "Content-Disposition": f'attatchment; filename=contrato/{hoje.strftime("%d/%m")}.csv"'
            },
        )
...
def feriasview(request):
    ...
    ferias = feriaspj.objects.filter(quitado=False, funcionario_id__in=aa).order_by(
        "vencimento"
    )
    nome = request.POST.get("textbox")
    option = request.POST.get("option")
    ...
    return render(
        request,
        "portaria/pj/feriasview.html",
        {"ferias": ferias, "dias": dias, "hoje": hoje},
    )
...
def feriasagen(request, idfpj):
    ...
    if agen1 and agen2:
        try:
            ...
            feriaspj.objects.filter(pk=fer.id).update(
                agendamento_ini=agenparse1, agendamento_fim=agenparse2
            )
        except ValueError:
            messages.error(request, "Por favor, digite uma data válida")
...
def feriasquit(request, idfpj):
    ...
    try:
        feriaspj.objects.filter(pk=fer.id).update(
            quitado=True, dt_quitacao=timezone.now()
        )
    except ObjectDoesNotExist:
        messages.error(request, "Erro")
        return redirect("portaria:feriasview")
...
def manupendentes(request):
    ...
    if request.method == "GET":
        ...
        if placa:
            qs = ManutencaoFrota.objects.filter(
                status="PENDENTE", veiculo__prefixoveic=placa
            ).order_by("dt_entrada")
    if request.method == "POST":
        osid = request.POST.get("os")
        try:
            isos = get_object_or_404(ManutencaoFrota, pk=osid)
            ManutencaoFrota.objects.filter(pk=isos.id).update(
                status="CONCLUIDO", autor=request.user
            )
        except ObjectDoesNotExist:
            ...
            return redirect("portaria:manupendentes")
    return render(
        request, "portaria/frota/manutencaopendencia.html", {"qs": qs, "qs2": qs2}
    )
...
def manuentrada(request, placa_id):
    array = []
    gp_servs = TipoServicosManut.objects.values_list(
        "grupo_servico", flat=True
    ).distinct()
    for q in gp_servs:
    ...
    autor = request.user
    ult_manu = (
        Veiculos.objects.filter(pk=placa.codigoveic)
        .values_list("manutencaofrota__dt_saida", flat=True)
        .order_by("-manutencaofrota__dt_saida")
        .first()
    )
    if ult_manu == None:
        ult_manu = timezone.now()
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                manu.save()
                ServJoinManu.objects.create(
                    id_svs_id=tp_sv, autor=autor, id_os_id=manu.id
                )
                if count:
                    ncount = int(count)
                    if ncount > 0:
                ...
                raise e
            messages.success(
                request, f"Cadastro de manutenção do veículo {placa} feito com sucesso!"
            )
            return redirect("portaria:manutencaoprint", osid=manu.id)
    return render(
        request,
        "portaria/frota/manuentrada.html",
        {"placa": placa, "form": form, "array": array},
    )
    ...
    if get_os.dt_saida == None:
        if request.method == "POST":
            ...
            if dtsaida and vlmao and vlpeca:
                try:
                    array = []
                    date_dtsaida = datetime.datetime.strptime(
                        dtsaida, "%Y-%m-%d"
                    ).date()
                    for i in range(0, len(idsvs)):
                        array.append(
                            {
                                "idsvs": idsvs[i],
                                "vlmao": vlmao[i].replace(",", "."),
                                "vlpeca": vlpeca[i].replace(",", "."),
                                "prod": prod[i],
                                "forn": forn[i],
                    ...
                    messages.error(request, "Por favor, digite uma data válida")
                    return render(
                        request, "portaria/frota/manusaida.html", {"get_os": get_os}
                    )
                else:
                    between_days = (date_dtsaida - get_os.dt_entrada).days
                    ManutencaoFrota.objects.filter(pk=get_os.id).update(
                    ...
                    for q in array:
                        try:
                            ServJoinManu.objects.filter(pk=q["idsvs"]).update(
                                valor_maodeobra=q["vlmao"],
                                valor_peca=q["vlpeca"],
                                produto=q["prod"],
                                fornecedor=q["forn"],
                                feito=q["feito"],
                    ...
                    messages.success(
                        request,
                        f"Saída cadastrada para OS {get_os.id}, placa {get_os.veiculo}",
                    )
                    return redirect("portaria:manutencaoview")
        return render(
            request, "portaria/frota/manusaida.html", {"get_os": get_os, "svsss": svsss}
        )
    else:
...
def agendamentomanu(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if placa and tp_sv:
                try:
                    os = ManutencaoFrota.objects.create(
                        veiculo_id=placa.codigoveic,
                        autor=autor,
                        motorista=autor,
                        tp_manutencao="PRE",
                    )
                    for q in tp_sv:
                        ServJoinManu.objects.create(
                            id_svs_id=q, autor=autor, id_os_id=os.id
                        )
                except Exception as e:
                    print(e)
                else:
...
def updatemanu(request, osid):
    ...
    ult_manu = (
        Veiculos.objects.filter(pk=os.veiculo.codigoveic)
        .values_list("manutencaofrota__dt_saida", flat=True)
        .order_by("-manutencaofrota__dt_saida")
        .first()
    )
...
class ManutencaoListView(generic.ListView):
    ...
    def get_queryset(self):
        qs = ManutencaoFrota.objects.filter(
            dt_saida=None,
        ).order_by("tp_manutencao", "dt_entrada")
        try:
            placa = self.request.GET.get("isplaca")
            if placa:
                qs = (
                    ManutencaoFrota.objects.filter(
                        Q(veiculo__prefixoveic=placa) | Q(id=placa)
                    )
                    .exclude(dt_saida__isnull=False)
                    .order_by("dt_entrada")
                )
        except ObjectDoesNotExist:
            raise Exception("Valor digitado inválido")
        ...
        metrics = (
            ...
            .annotate(
                ...
                concluido=Count(
                    ...
                    filter=Q(
                        status="CONCLUIDO",
                        dt_entrada__month=datetime.datetime.now().month,
                        dt_entrada__year=datetime.datetime.now().year,
                    ),
                ),
            )
            .aggregate(
                prev=Sum("preventiva"), corr=Sum("corretiva"), conc=Sum("concluido")
            )
        )
...
def disponibilidade_frota(request: WSGIRequest) -> HttpResponse:
    if request.user.is_authenticated:
        ...
        veiculos: list[Veiculos] = list(
            Veiculos.objects.filter(filial=filial_selecionada).order_by("prefixoveic")
        )
        movimentos: list[DisponibilidadeFrota] = list(
            DisponibilidadeFrota.objects.filter(
                filial=filial_selecionada, data_preenchimento=hoje
            )
        )
        ...
        for veiculo in veiculos:
            ...
            if movimento:
                if movimento.status == "PARADO":
                    dias_restantes = (
                        movimento.data_liberacao - datetime.date.today()
                    ).days
                    if dias_restantes >= 0:
                        bloqueio_parado.append(veiculo)

                if movimento.status == "PREVENTIVO":
                    dias_restantes = (
                        movimento.data_previsao - datetime.date.today()
                    ).days
                    if dias_restantes >= 0:
                        bloqueio_preventivo.append(veiculo)

            # Transformando numero em um codigo
            veiculo.codigotpveic = Veiculos.CODIGOTPVEIC_CHOICES[
                int(veiculo.codigotpveic) - 1
            ][1]

        if request.method == "POST":
            dict: dict = request.POST.dict()  # Transformando o request em dict
            cor: str = [*dict][1]  # Selecionando a cor vinda do request
            veiculo: Veiculos = Veiculos.objects.get(
                codigoveic=dict[cor]
            )  # {'COR SELECIONADA': CODIGOVEIC}
            ...
            return HttpResponseRedirect(
                f"/frota/disponibilidade-frota?filiais={filial_selecionada}"
            )

        return render(
            request,
            "portaria/frota/disponibilidade_frota.html",
            {
                "filiais": filiais,
                "veiculos": veiculos,
                "placas_movimentos": placas,
...
def manutencaoprint(request, osid):
    os = get_object_or_404(ManutencaoFrota, pk=osid)
    aa = ServJoinManu.objects.filter(id_os=os.id).annotate(
        grp=F("id_svs__grupo_servico"), svs=F("id_svs__tipo_servico")
    )
...
def monitticket(request):
    ...
    tkts = TicketMonitoramento.objects.filter(
    ...
    if request.method == "POST":
        ...
        if tkt:
            tkts = TicketMonitoramento.objects.filter(
                Q(pk=tkt) | Q(nome_tkt__icontains=tkt)
        ...
        if employee1 and not period1 and not period2:
            ...
            tkts = TicketMonitoramento.objects.filter(
                dt_abertura__lte=period2, dt_abertura__gte=period1, status__in=stts
            )
        elif period1 and period2 and employee1:
            tkts = TicketMonitoramento.objects.filter(
                dt_abertura__lte=period2,
                dt_abertura__gte=period1,
                responsavel_id=employee1,
                status__in=stts,
            )
        elif not period1 and not period2 and not employee1 and stts:
            messages.error(
                request, "Por gentileza selecione o período ou o funcionário"
            )
        return render(
            request,
            "portaria/monitoramento/monitticket.html",
            {"tkts": tkts, "employee": employee},
        )
    return render(
        request,
        "portaria/monitoramento/monitticket.html",
        ...
        if cte and gar and tp_docto:
            ...
            try:
                ...
                if len(res) > 1:
                    ...
                    modal = EmailOcorenciasMonit.objects.filter(
                        rsocial=res[0]["remetente"], ativo=1
                    )
                    return render(
                        request,
                        "portaria/monitoramento/tktcreate.html",
                        {
                            "editor": editor,
                            "users": users,
                            "res": res,
...
def tktview(request, tktid):
    ...
    if request.method == "POST":
        ...
        if nstts != "selected":
            ...
            if tkt and tkt.categoria != "Aguardando Recebimento":
                ...
                messages.info(
                    request, f"Ticket {tkt.id} alterado para {nstts} com sucesso."
                )
            else:
                messages.error(
                    request,
                    f"Não autorizado a mudança de status para {nstts}, por gentileza mude a primeira categoria.",
                )
                return redirect("portaria:monitticket")
    ...
    return render(
        request,
        "portaria/monitoramento/ticketview.html",
        {"form": form, "editor": editor, "opts": opts, "stts": stts, "teste": teste},
    )
...
def tktmetrics(request):
    ...
    metrics = TicketMonitoramento.objects.annotate(
        total=Count(
            ...
            filter=~Q(
                ...
                dt_abertura__year=datetime.datetime.now().year,
            ),
        ),
        hoje=Count("id", filter=Q(dt_abertura=hj)),
        andamento=Count(
            "id",
            filter=Q(
                status="ANDAMENTO",
        ...
        total1=Sum(F("andamento") + F("aberto")),
        hoje1=Sum("hoje"),
    ...
    totfunc = (
        User.objects.filter(groups__name="monitoramento")
        .annotate(
            total=Count(
                "responsavel__id",
                filter=Q(responsavel__status__in=["ABERTO", "ANDAMENTO"]),
            ),
            diario=Count(
                "responsavel__id",
                filter=Q(responsavel__dt_abertura=datetime.date.today()),
            ),
            concluido=Count(
                "responsavel__id",
                filter=Q(
    ...
    totfuncself = (
        User.objects.filter(
            groups__name="monitoramento", responsavel__responsavel=request.user
        )
        .annotate(
            total=Count(
                "responsavel__id",
    ...
    return render(
        request,
        "portaria/monitoramento/ticketmetrics.html",
        {"metrics": metrics, "totfunc": totfunc, "totfuncself": totfuncself},
    )
        ...
        if empresa and mail:
            ...
            if count:
                ...
                for c in range(0, ncount):
                    ...
                    if mails:
                        try:
                            EmailOcorenciasMonit.objects.create(
                                email=mails, rsocial=empresa, ativo=1
                            )
                        except Exception as e:
                            messages.error(
                                request,
                                f"Erro ao cadastrar os emails, ErrorType:{type(e).__name__}, Error: {e}",
                            )
                        finally:
...
def chamado(request):
    metrics = (
        TicketChamado.objects.exclude(Q(status="CANCELADO") | Q(status="CONCLUIDO"))
        .annotate(
            fis=Count("id", filter=Q(servico="FISCAL")),
            praxio=Count("id", filter=Q(servico="PRAXIO")),
            des=Count("id", filter=Q(servico="DESCARGA")),
            comp=Count("id", filter=Q(servico="COMPROVANTE")),
            hoje=Count("id", filter=Q(dt_abertura__date=datetime.date.today())),
            andamento=Count("id", filter=Q(status="ABERTO") | Q(status="ANDAMENTO")),
        )
        .aggregate(
            fis1=Sum("fis"),
            praxio1=Sum("praxio"),
...
def chamadopainel(request):
    ...
    form = TicketChamado.objects.all().exclude(
        Q(status="CANCELADO") | Q(status="CONCLUIDO")
    )
    if request.method == "POST":
        srctkt = request.POST.get("srctkt")
        if srctkt:
            form = TicketChamado.objects.filter(pk=srctkt).exclude(
                Q(status="CANCELADO") | Q(status="CONCLUIDO")
            )
            return render(
                request, "portaria/chamado/chamadopainel.html", {"form": form}
            )
...
def chamadonovo(request):
    ...
    return render(
        request,
        "portaria/chamado/chamadonovo.html",
        {
            "editor": editor,
            "stts": stts,
            "dp": dp,
...
def chamadodetail(request, tktid):
    ...
    if request.method == "POST":
        ...
        try:
            if nsubject != form.assunto:
                TicketChamado.objects.filter(pk=form.tkt_ref_id).update(
                    nome_tkt=nsubject,
    ...
    return render(
        request,
        "portaria/chamado/chamadodetail.html",
        {
            "form": form,
            "editor": editor,
            "stts": stts,
...
def chamadodelete(request, tktid):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"Ticket {tktid} deletado com sucesso!"
        )  # HttpResponse(f'<h2>Ticket {tktid} deletado com sucesso!</h2>')
        return redirect("portaria:chamadopainel")


def chamado_concluido(request):
    form = TicketChamado.objects.filter(
        status="CONCLUIDO",
        dt_abertura__year=datetime.datetime.now().year,
        dt_abertura__month=datetime.datetime.now().month,
    )
    if request.method == "POST":
        date1 = datetime.datetime.strptime(
            request.POST.get("date1"), "%Y-%m-%d"
        ).replace(hour=00, minute=00)
        date2 = datetime.datetime.strptime(
            request.POST.get("date2"), "%Y-%m-%d"
        ).replace(hour=23, minute=59)
        if date1 and date2:
            form = TicketChamado.objects.filter(
                status="CONCLUIDO", dt_abertura__lte=date2, dt_abertura__gte=date1
            )
            return render(
                request, "portaria/chamado/chamadosconcluidos.html", {"form": form}
            )
...
def etiquetas(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            for i in res:
                try:
                    check = EtiquetasDocumento.objects.filter(
                        garagem=ga,
                        tp_doc=doc,
                        nr_doc=i["conhecimento"],
                        nota=i["nota_fiscal"],
                        volume=i["volumes"],
                    )
                    if not check:
                        EtiquetasDocumento.objects.create(
                            garagem=ga,
                            tp_doc=doc,
                            nr_doc=i["conhecimento"],
                            nota=i["nota_fiscal"],
    ...
    return render(
        request,
        "portaria/etiquetas/etiquetas.html",
        {"gachoices": gachoices, "docchoices": docchoices},
    )
...
def createetiquetas(request):
    ...
    if lista:
        if (
            request.POST.get("getbtnprint")
            and request.POST.get("getbtnprint") == "Click"
        ):
...
def contagemetiquetas(request):
    ...
    return render(
        request,
        "portaria/etiquetas/contagemetiquetas.html",
        {"gachoices": gachoices, "docchoices": docchoices},
    )
...
def bipagemetiquetas(request):
    ...
    docs = EtiquetasDocumento.objects.filter(
        nr_doc=dict["cte"], nota=dict["nf"], garagem=dict["ga"], tp_doc=dict["tp_doc"]
    )
    cont = 0
    for k in docs:
        cont += k.volume
                ...
                if len(test) == cont:
                    for i in test:
                        if not BipagemEtiqueta.objects.filter(cod_barras=i):
                            ...
                            if check:
                                BipagemEtiqueta.objects.create(
                                    cod_barras=i,
                                    nota=i[-10:],
                                    doc_ref=docs[0],
                                    autor=request.user,
                                ...
                                messages.error(
                                    request,
                                    f"{i} não pertence ao romaneio, gentileza verificar.",
                                )
                                return HttpResponse(
                            ...
                            check = docs.filter(nota=i[-10:])
                            if check:
                                BipagemEtiqueta.objects.filter(cod_barras=i).update(
                    ...
                    messages.error(
                        request,
                        "Quantidade de notas enviadas inválida, gentileza verificar.",
                    )
    ...
    return render(
        request,
        "portaria/etiquetas/bipagemetiquetas.html",
        {"docs": docs, "nrdoc": dict["cte"], "cont": cont},
    )
...
def retornoetiqueta(request):
    ...
    if request.method == "POST":
        ...
        for q in nflist:
            if (
                EtiquetasDocumento.objects.filter(nota=q[-10:])
                and RetornoEtiqueta.objects.filter(
                    nota_fiscal=q,
                    saida__month=datetime.datetime.now().month,
                    saida__year=datetime.datetime.now().year,
                ).exists()
                == False
            ):
    ...
    return render(
        request, "portaria/etiquetas/retornoetiqueta.html", {"gachoices": gachoices}
    )
...
def etiquetas_palete(request):
    ...
    last = (
        EtiquetasPalete.objects.all()
        .order_by("-id")
        .values_list("cod_barras", flat=True)
        .first()
    )
    ...
    if request.method == "POST":
        ...
        if ga and vol and cli:
            etq = EtiquetasPalete.objects.create(
                cod_barras=new,
                filial=ga,
                volumes=vol,
                cliente=cli,
    ...
    return render(
        request,
        "portaria/etiquetas/etiquetas_palete.html",
...
def bipagem_palete(request):
    ...
    if request.method == "POST":
        ...
        if code and ga != "Selecione...":
            try:
                ...
                bip = BipagemPalete.objects.create(
                    filial=ga,
                    cod_barras=code,
                    volume_conf=vol,
                    autor=request.user,
    ...
    return render(
        request, "portaria/etiquetas/bipagem_palete.html", {"gachoices": gachoices}
    )
...
def etqrelatorio(request):
    gachoices = TicketMonitoramento.GARAGEM_CHOICES
    return render(
        request, "portaria/etiquetas/etiquetasrelatorio.html", {"gachoices": gachoices}
    )
...
def painelromaneio(request):
    context = RomXML.objects.filter(
        pub_date__month=datetime.datetime.now().month,
        pub_date__year=datetime.datetime.now().year,
    ).order_by("-pub_date")
    getrem = (
        RomXML.objects.filter(
            pub_date__month=datetime.datetime.now().month,
            pub_date__year=datetime.datetime.now().year,
        )
    ...
    if request.method == "GET":
        ...
        if dt1 and dt2:
            dt1p = datetime.datetime.strptime(dt1, "%Y-%m-%d").replace(
                hour=00, minute=00
            )
            dt2p = datetime.datetime.strptime(dt2, "%Y-%m-%d").replace(
                hour=23, minute=59
            )
            if rem:
                context = RomXML.objects.filter(
                    pub_date__gte=dt1p, pub_date__lte=dt2p, remetente=rem
                ).order_by("-pub_date")
            else:
                context = RomXML.objects.filter(
                    pub_date__gte=dt1p, pub_date__lte=dt2p
                ).order_by("-pub_date")
        ...
        if romidd:
            if tp_dld == "Completo":
                romaneio = SkuRefXML.objects.filter(xmlref__id__in=romidd).annotate(
                    ...
                    municipio1=F("xmlref__municipio"),
                    uf1=F("xmlref__uf"),
                    codigo1=F("codigo"),
                    qnt_un1=F("qnt_un"),
                    desc_prod1=F("desc_prod"),
                    rem=F("xmlref__remetente"),
                    romaneio_id=F("xmlref_id"),
                    volume=F("xmlref__volume"),
                )
            elif tp_dld == "Simples":
                ...
                romaneio = SkuRefXML.objects.filter(xmlref__id__in=romidd).annotate(
                    ...
                    desc_prod1=F("desc_prod"),
                    rem=F("xmlref__remetente"),
                    romaneio_id=F("xmlref__nota_fiscal"),
                    volume=F("xmlref__volume"),
                    valor=F("xmlref__vlr_nf"),
                    tp_un1=F("tp_un"),
                    peso=F("xmlref__peso"),
                    tp_vol1=F("tp_vol"),
                    qnt_vol1=F("qnt_vol"),
            ...
            try:
                ...
                messages.error(
                    request, f"Algo deu errado, verifique e tente novamente."
                )
                return redirect("portaria:painelromaneio")
            else:
                if sheet:
    ...
    return render(
        request,
        "portaria/etc/painelromaneio.html",
        {"context": context, "getrem": getrem},
    )


def entradaromaneio(request):
    if request.method == "POST":
        emissao = datetime.datetime.strptime(
            request.POST.get("emissao"), "%Y-%m-%dT%H:%M"
        )
...
def entradaxml(request, args=None):
    ...
    for file in files:
        ...
        try:
            ...
            dest_cnpj = (
                mydoc.getElementsByTagName("dest")[0]
            ...
            if dest_cnpj not in cnpjs:
                ...
                dhEmi = dateformat.format(
                    datetime.datetime.strptime(
                        mydoc.getElementsByTagName("dhEmi")[0].firstChild.nodeValue,
                        "%Y-%m-%dT%H:%M:%S%z",
                    ),
                    "Y-m-d H:i",
                )
                try:
                    rem = (
                        mydoc.getElementsByTagName("emit")[0]
                ...
                peso = (
                    mydoc.getElementsByTagName("transp")[0]
                ...
                vlr_nf = (
                    mydoc.getElementsByTagName("total")[0]
...
def romxmltoexcel(*romaneio, tp_dld):
    ...
    if tp_dld == "Completo":
        ...
        dt = (
            pdr.pivot_table(
                index=["remetente", "nota", "valor", "peso", "desc", "tp_un", "tp_vol"],
    ...
    response[
        "Content-Disposition"
    ] = f'attachment; filename="{datetime.datetime.today()}.xlsx"'
...
def solictransfpalete(request):
    ...
    if request.method == "POST":
        ...
        if qnt <= PaleteControl.objects.filter(loc_atual=ori, tp_palete=tp_p).count():
            ...
            solic_id = (
                str(time).replace(":", "").replace(" ", "").replace("-", "") + plc[5:]
            )
            if len(plc) <= 7:
                for q in range(0, qnt):
                    x = PaleteControl.objects.filter(
                        loc_atual=ori, tp_palete=tp_p
                    ).first()
                    solic = SolicMovPalete.objects.create(
                        solic_id=solic_id,
                        palete=x,
                        data_solic=currentTime,
                        origem=ori,
                        destino=des,
                        placa_veic=plc,
                        autor=request.user,
                ...
                messages.success(
                    request,
                    f"{qnt} palete(s) | Aguardando o recebimento de paletes de {ori} para {des}",
                )
                ...
                return render(
                    request,
                    "portaria/palete/transfpaletes.html",
                    {"form": form, "garagem": garagem},
                )
...
def transfdetalhe(request, solic_id):
    ...
    quantity = (
        SolicMovPalete.objects.filter(solic_id=solic_id)
        .values("solic_id")
        .annotate(Count("solic_id"))
    )
    qty = quantity[0]
    ...
    data = {
        ...
        "Data Solicitação": datetime.datetime.strftime(
            mov.data_solic, "%d/%m/%Y %H:%m"
        ),
...
def paineltransf(request):
    ...
    if not origem is None:
        filtro["origem"] = origem
    if not destino is None:
        filtro["destino"] = destino
    if not placa_veiculo is None and placa_veiculo != "":
        filtro["placa_veic"] = placa_veiculo
    if not autor is None and autor != "":
        filtro["autor__username"] = autor

    form = (
        SolicMovPalete.objects.filter(**filtro)
        .values(
            "solic_id",
            "destino",
            "origem",
    ...
    return render(
        request, "portaria/palete/paineltransf.html", {"form": form, "filiais": filiais}
    )
...
def transfpalete(request):
    ...
    if request.method == "GET":
        ...
        if solic_id:
            ...
            return render(
                request, "portaria/palete/recpaletes.html", {"solic": form, "qty": qty}
            )
    ...
    if request.method == "POST":
        ...
        if movPalete.count() == 0:
            messages.error(
                request, f"Não existem mais pallets disponíveis nessa transferência"
            )
        ...
        if int(qnt) <= int(movPalete.count()) or int(movPalete.count()) == int(qnt):
            ...
            for q in range(0, qnt):
                ...
                if x.loc_atual != des:
                    ...
                    solic = MovPalete.objects.create(
                        solic_id=solic_id,
                        palete=x,
                        data_solic=datetime.datetime.strftime(
                            dt_parse, "%Y-%m-%d %I:%M"
                        ),
                        data_receb=dt_receb,
                        origem=ori,
            ...
            messages.error(
                request, "A quantidade recebida é superior ao que foi enviado"
            )
            return render(request, "portaria/palete/recpaletes.html")
    return render(request, "portaria/palete/recpaletes.html")
...
def painelmov(request):
    ...
    filtro = {}
    if not data_solic is None and data_solic != "":
        filtro["data_solic"] = data_solic
    if not data_receb is None and data_receb != "":
        filtro["data_receb"] = data_receb
    if not origem is None:
        filtro["origem"] = origem
    if not destino is None:
        filtro["destino"] = destino
    ...
    form = (
        MovPalete.objects.filter(**filtro)
        .values(
            "solic_id",
            "destino",
            "origem",
    ...
    return render(
        request,
        "portaria/palete/painelmov.html",
        {"form": form, "filiais": filiais, "today": today},
    )
...
def get_nfpj_mail(request):
    ...
    dt_envio_nf = datetime.datetime.strptime(
        dt_envio_nf + " 00:00:00", "%Y-%m-%d %H:%M:%S"
    )

    filter = {"id__in": func_id, "ativo": True}
    ...
    for q in array:
        try:
            send_mail(
                subject=title,
                message=text.format(
                    f"{q.filial.nome} - {q.filial.uf}",
                    q.nome,
                    q.salario,
                    q.faculdade,
                    q.ajuda_custo + q.aux_moradia,
                    q.cred_convenio,
                    q.outros_cred,
                    q.adiantamento,
                    q.desc_convenio,
                    ...
                    dt_1.strftime("%d/%m/%Y"),
                    dt_2.strftime("%d/%m/%Y"),
                    dt_pgmt.strftime("%d/%m/%Y"),
                    q.aux_moradia,
                    q.pix or "Não Informado",
                    dt_envio_nf.strftime("%d/%m/%Y") or "Não Informado",
                ),
            ...
            MailsPJ.objects.create(
                funcionario_id=q.id,
                data_pagamento=dt_pgmt,
...
def get_pj13_mail(request):
    ...
    if getperiod == "pgto_parcela_1":
        allfuncs = FuncPj.objects.filter(
            ativo=True, pj13__pgto_parc_1__isnull=False, pj13__pgto_parc_2__isnull=True
        )
        for q in allfuncs:
            query = FuncPj.objects.filter(pk=q.id).annotate(
                valor=F("pj13__valor"),
...
def get_portaria_csv(request):
    ...
    if fil and not data1 and not data2:
        cadastro = (
            ...
            .annotate(
                hr_chegada_fmt=Cast(
                    TruncMinute("hr_chegada", DateTimeField()), CharField()
                ),
                hr_saida_fmt=Cast(
                    TruncMinute("hr_chegada", DateTimeField()), CharField()
                ),
        ...
        dateparse = datetime.datetime.strptime(data1, "%Y-%m-%d").replace(
            hour=00, minute=00
        )
        dateparse1 = datetime.datetime.strptime(data2, "%Y-%m-%d").replace(
            hour=23, minute=59
        )
        cadastro = (
            Cadastro.objects.all()
            ...
            array.append(
                {
                    "origem": q.origem,
                    "destino": q.destino,
                    "data_ult_mov": q.data_ult_mov,
                    "placa": q.placa_veic,
                    "autor": q.autor,
                    "tipo": q.palete.tp_palete if q.palete else "DEVOLVIDO",
                }
            )
...
def get_manu_csv(request):
    ...
    try:
        ...
        manutencao = ManutencaoFrota.objects.all().filter(
            dt_entrada__gte=dateparse, dt_entrada__lte=dateparse1
        )
        for q in manutencao:
            array.append(
                {
                    "id": q.id,
                    "veiculo": q.veiculo.prefixoveic,
                    "motorista": q.motorista,
                    "tp_manu": q.tp_manutencao,
                    "local_manu": q.local_manu,
                    "ult_manu": q.dt_ult_manutencao,
                    "entrada": q.dt_entrada,
                    "inicio": q.dt_ini_manu,
                    "saida": q.dt_saida,
                    "dias_parado": q.dias_veic_parado,
                    "km": q.km_atual,
                    "filial": q.filial,
                    "socorro": q.socorro,
                    "prev_entrega": q.prev_entrega,
                    "status": q.status,
        ...
        response[
            "Content-Disposition"
        ] = f'attachment; filename="{dateparse}-{dateparse1}.xlsx"'
        writer = pd.ExcelWriter(response, engine="xlsxwriter")
        df.to_excel(writer, "sheet1", index=False)
        writer.save()
        return response
...
def get_nfpj_csv(request):
    ...
    for q in qnt_funcs:
        ...
        if len(nfs) > 0:
            for nf in nfs:
                query = (
                    FuncPj.objects.filter(pk=q.id, nfservicopj=nf)
                    .annotate(
                        ...
                        data_emissao=Coalesce(
                            "nfservicopj__data_emissao", datetime.date.today()
                        ),
                    )
                    .annotate(
                        total=(
...
def get_checklist_csv(request):
    ...
    try:
        ...
        query = ChecklistFrota.objects.filter(
            datachecklist__lte=fim, datachecklist__gte=ini
        )
        if query:
            for q in query:
                array.append(
                    {
                        "id": q.idchecklist,
                        "emissao": q.datachecklist,
                        "placa": q.placaveic.prefixoveic,
                        "motorista": q.motoristaveic.nome,
                        "filial": q.get_filial_display(),
                        "km anterior": q.kmanterior,
                        "km atual": q.kmatual,
                        "horimetro": q.horimetro,
                        "autor": q.autor.username,
                    }
                )
            df = pd.DataFrame(array)
...
def get_ferias_csv(request):
    try:
        ...
        ferias = (
            ...
            .filter(ultimas_ferias_ini__gte=ini, ultimas_ferias_fim__lte=fim)
        )
        for q in ferias:
            writer.writerow(q)
        return response
...
def get_bonus_csv(request):
    ...
    try:
        ...
        response = HttpResponse(
            content_type="text/csv",
            headers={
                "Content-Disposition": f'attatchment; filename=bonus/{hoje.strftime("%d/%m")}.csv"'
            },
        )
        response.write("\ufeff".encode("utf8"))
        writer = csv.writer(response)
    ...
    if request.method == "POST":
        ...
        if get_xlsx:
            ...
            for q in range(len(array)):
                ...
                response.write(
                    textwrap.wrap(array[q]["nome_do_cliente"], 40)[0].ljust(40, " ")
                )
                ...
                response.write(
                    textwrap.wrap(str(array[q]["nome_dest"]), 40)[0].ljust(40, " ")
                )
                response.write(
                    textwrap.wrap(str(array[q]["end_dest"]), 40)[0].ljust(40, " ")
                )
                response.write(
                    textwrap.wrap(str(array[q]["cidade"]), 30)[0].ljust(30, " ")
                )
        ...
        try:
            ...
            if parsed_email.is_multipart():
                for part in parsed_email.walk():
                    ...
                    if filename:
                        path = (
                            settings.STATIC_ROOT + "/monitoramento/" + str(hoje) + "/"
                        )
                        ...
                        if os.path.exists(os.path.join(path)):
                            ...
                            try:
                                os.rename(
                                    locimg, os.path.join(path, (str(rr) + filename))
                                )
                            except Exception as e:
                                os.rename(
                                    locimg,
                        ...
                        item = os.path.join(
                            "/static/monitoramento/" + str(hoje) + "/",
                            (str(rr) + filename),
                        )
                        aa = (
                            '<div class="mailattatch"><a href="'
                            + item
                            + '" download><img src="/static/images/downicon.png" width="40"><p>'
                            + filename
                            + "</p></a></div>"
                        )
                        attatch += aa
            ...
            try:
                ...
                if re.findall(r"<(.*?)>", e_from):
                    e_from = re.findall(r"<(.*?)>", e_from)[0]

                e_to = parsed_email["To"]
                if re.findall(r"<(.*?)>", e_to):
                    e_to = re.findall(r"<(.*?)>", e_to)[0]
                ...
                w_body = (
                    '<div class="container chmdimg">' + htbody.decode(cs) + "</div>"
                )
                if re.findall(pattern2, w_body):
                    for q in re.findall(pattern2, w_body):
                        ...
                        if re.findall(f"/media/django-summernote/{str(hoje)}/", q):
                            ...
                            f"/static/images/macros-monit/\w+[.]\w+.(?i:jpeg|jpg|gif|png|bmp)",
                            q,
                        ):
                            new_cid = os.path.join(
                                settings.STATIC_URL
                                ...
                                (
                                    str(rr)
                                    + new[0].split(f"cid:/static/images/macros-monit/")[
                                        1
                                    ]
                                ),
                            )
                        elif re.findall(
                            f"/static/images/macros-monit/\w+.(?i:jpeg|jpg|gif|png|bmp)",
                            q,
                        ):
                            new_cid = os.path.join(
                                settings.STATIC_URL
                            ...
                            if re.findall(f"/media/django-summernote/{str(hoje)}/", q):
                                ...
                                new_cid = os.path.join(
                                    ...
                                    (
                                        str(rr)
                                        + new[0].split(
                                            f"cid:/static/images/macros-monit/"
                                        )[1]
                                    ),
                                )
                            elif re.findall(
...
def replymail_monitoramento(request, tktid, area, myfile):
    ...
    keyga = {v: k for k, v in TicketMonitoramento.GARAGEM_CHOICES}
    getmailfil = EmailOcorenciasMonit.objects.filter(
        rsocial=orig.tkt_ref.get_filial_display(), ativo=1
    )
    mailfil = ""
    for i in getmailfil:
        mailfil += i.email + ", "
    ...
    if request.method == "POST":
        ...
        if myfile is not None:
            for q in myfile:
                ...
                if os.path.exists(os.path.join(path)):
                    ...
                    try:
                        ...
                        os.rename(
                            locimg,
                            os.path.join(
                                path, (str(rr) + str(q) + str(random.randint(1, 100)))
                            ),
                        )
                else:
                    os.mkdir(path=path)
                ...
                item = os.path.join(
                    "/static/monitoramento/" + str(hoje) + "/", (str(rr) + str(q))
                )
                aatt = (
                    '<div class="mailattatch"><a href="'
                    + item
        ...
        if re.findall(pattern, msg):
            for q in re.findall(pattern, msg):
                ...
                msgimg = MIMEImage(
                    img_data, name=os.path.basename(media), _subtype="jpg"
                )
        ...
        signimg = open(
            f"{settings.STATIC_ROOT}/images/macros-monit/{request.user}.jpg", "rb"
        ).read()
        ...
        if orig.ult_resp:
            msgmail2222 = (
                '<div class="container chmdimg">'
                + msgm
                + sign
                + "</div>"
                + orig.ult_resp.split("<p>Anterior</p><hr>")[0]
            )
            msgmail1234 = (
                '<div class="container chmdimg">'
                + msg
                + sign
                + "</div>"
                + orig.ult_resp_html.split("<p>Anterior</p><hr>")[0]
            )
        else:
            msgmail2222 = (
                '<div class="container chmdimg">'
            ...
            for q in re.findall(pattern2, msgmail2222):
                ...
                for i in msgimg2.walk():
                    if i["Content-ID"] not in arrayzz:
                        msg1.attach(
                            msgimg2
                        )  # causa dos attach pelas repetições duplicadas
                        msgmail2222 = msgmail2222.replace(
                            (f'="{q}"'), f'="cid:{os.path.basename(q)}"'
                        )
                        arrayzz.append(i["Content-ID"])
        msgmail2222 = msgmail2222.replace(
            f'<br><img src="/static/images/macros-monit/{request.user}.jpg" width="600">',
            f'<br><img src="cid:{request.user}.jpg" width="600">',
        )
        ...
        if orig.ult_resp is not None:
            ...
            bb = (
                ...
                + str(request.user)
                + " -- "
                + str(dateformat.format(timezone.now(), "d-m-Y H:i"))
                + "<br>"
                + msgmail1234
                + "<br>"
                + attatch
        ...
        try:
            ...
            sm.sendmail(
                get_secret("EUSER_MN"), orig.cc.split(";"), msg1.as_string()
            )  #############
            print("mandou o email")
            EmailMonitoramento.objects.filter(pk=orig.id).update(
                ult_resp=aa,
                ult_resp_html=bb,
                ult_resp_dt=dateformat.format(timezone.now(), "Y-m-d H:i"),
            )
        except Exception as e:
            print(f"ErrorType:{type(e).__name__}, Error:{e}")
        else:
...
def createtktandmail(request, **kwargs):
    ...
    send = (
        ["IGOR.ROSARIO@BORA.COM.BR", "ROBERT.DIAS@BORA.COM.BR", request.user.email]
        + kwargs["cc"].split(";")
        + mailfil.split(",")
    )
    ...
    if re.findall(pattern, kwargs["msg"]):
        for q in re.findall(pattern, kwargs["msg"]):
            ...
            msgimg.add_header("Content-ID", f"{media}")
            msgmail = msgmail.replace(
                ('<img src="' + media + '"'), f'<img src="cid:{media}" '
            )
            msg1.attach(msgimg)
    ...
    try:
        ...
        try:
            ...
            tkt = TicketMonitoramento.objects.create(
                nome_tkt=kwargs["assunto"],
                dt_abertura=timezone.now(),
                responsavel=User.objects.get(username=kwargs["resp"]),
                solicitante=request.user,
                remetente=kwargs["rem"],
                destinatario=kwargs["dest"],
                cte=kwargs["cte"],
                status="ABERTO",
                categoria="Aguardando Recebimento",
                msg_id=msg_id,
                filial=keyga[kwargs["filial"]],
                tp_docto=kwargs["tp_docto"],
            )
            mail = EmailMonitoramento.objects.create(
            ...
            messages.success(
                request, f"Email enviado e ticket criado com sucesso. ID: {tkt.id}"
            )
...
def createchamado(request, **kwargs):
    ...
    try:
        ...
        sm.set_debuglevel(1)
        sm.login(
            get_secret("EUSER_CH"), get_secret("EPASS_CH")
        )  ################### alterar
    except Exception as e:
        messages.error(request, f"ErrorType:{type(e).__name__}, Error:{e}")
        print(f"ErrorType:{type(e).__name__}, Error:{e}")
        ...
        try:
            tkt = TicketChamado.objects.create(
                nome_tkt=dict["assnt"],
                dt_abertura=timezone.now(),
                responsavel_id=dict["nresp"],
                servico=dict["nsvs"],
                solicitante=dict["solic"],
                status=dict["nstts"],
                msg_id=msg_id,
                departamento=dict["ndp"],
                filial=dict["nfil"],
            )
        except Exception as e:
            print(e)
        sm.sendmail(
            get_secret("EUSER_CH"),
            [get_secret("EUSER_CH")] + dict["solic"].split(";"),
            msg1.as_string(),
        )  ################### alterar
        notify.send(
            request.user,
        ...
        if request.method == "POST":
            ...
            if myfile is not None:
                for q in myfile:
                    ...
                    item = os.path.join(
                        "/static/chamados/" + str(hoje) + "/", (str(rr) + str(q))
                    )
                    aatt = (
                        '<div class="mailattatch"><a href="'
                        + item
            ...
            if orig.ult_resp:
                ...
                nmsg = (
                    ...
                    + msg
                    + "</div>"
                    + orig.mensagem.split('<div class="mailattatch">')[0]
                )
                nmsg2 = (
                    '<div class="container chmdimg">'
                    + msg2
            ...
            if re.findall(pattern2, nmsg):
                ...
                for q in re.findall(pattern2, nmsg):
                    ...
                    img_data = fp.read()
                    msgimg2 = MIMEImage(
                        img_data, name=os.path.basename(q), _subtype="jpg"
                    )
                    msgimg2.add_header("Content-ID", f"{os.path.basename(q)}")
                    for i in msgimg2.walk():
            ...
            try:
                ...
                EmailChamado.objects.filter(pk=orig.id).update(
                    ult_resp=aa,
                    ult_resp_html=bb,
                    ult_resp_dt=dateformat.format(timezone.now(), "Y-m-d H:i"),
                )
        ...
        for i in range(num_messages):
            ...
            try:
                ...
                parsed_email = email.message_from_bytes(
                    raw_email, policy=policy.compat32
                )
                # print('email parsed...')
            except Exception as e:
                ...
                if e_to:
                    ...
                    for q in re.findall(r"<(.*?)>", e_to):
                        if q not in [
                            "chamado.praxio@bora.tec.br",
                            "chamado.juridico@bora.tec.br",
                            "chamado.descarga@bora.tec.br",
                            "chamado.comprovantes@bora.tec.br",
                            "chamado.fiscal@bora.tec.br",
                            "chamado.manutencao@bora.tec.br",
                            "chamado.almoxarifado@bora.tec.br",
                        ]:
                            e_cc_a += q + ","
                e_cc = parsed_email["CC"]
                if e_cc:
                    ...
                    for q in re.findall(r"<(.*?)>", e_cc):
                        if q not in [
                            ...
                            e_cc_a += q + ","
                get_serv = (
                    str(parsed_email["Cc"]) + " " + str(parsed_email["To"])
                ).lower()
                ...
                if re.findall(pattern2, w_body):
                    for q in re.findall(pattern2, w_body):
                        ...
                        if re.findall(f"/media/django-summernote/{str(hoje)}/", q):
                            new_cid = os.path.join(
                                settings.STATIC_URL + "chamados/" + str(hoje) + "/",
                                (
                                    str(rr)
                                    + new[0].split(
                                        f"cid:/media/django-summernote/{str(hoje)}/"
                ...
                try:
                    ...
                    if form.exists():
                        ...
                        try:
                            ...
                            form.update(
                                ult_resp=aa, ult_resp_html=bb, ult_resp_dt=e_date
                            )
                    else:
                        try:
                            print("form dont exist")
                            newtkt = TicketChamado.objects.create(
                                solicitante=e_from,
                                servico=service,
                                nome_tkt=e_title,
                                dt_abertura=e_date,
                            ...
                            mensagem = (
                                "<hr>" + e_from + " -- " + e_date + w_body + attatch
                            )
                            newmail = EmailChamado.objects.create(
                                assunto=e_title,
                                mensagem=mensagem,
                                cc=e_cc_a,
                                dt_envio=e_date,
...
def mdfeporfilial(request):
    ...
    mailchoices = {
        ...
        "BAL": [""],
        "TCO": [
            "juliano.oliveira@borexpress.com.br",
            "lino.loureiro@borexpress.com.br",
        ...
        "CTG": [
            "ygor.henrique@borexpress.com.br",
            "samuel.santos@borexpress.com.br",
            "junior.morais@borexpress.com.br",
        ],
        "MCZ": [
            "rafael@bora.com.br",
            "carlos.eduardo@bora.com.br",
            "ronnielly@bora.com.br",
        ],
        "SSA": ["jaqueline.santos@bora.com.br", "brandao.alan@bora.com.br"],
        "NAT": [
            "ronnielly@bora.com.br",
            "lindalva@bora.com.br",
            "lidianne@bora.com.br",
        ],
        "SLZ": ["felipe@bora.com.br"],
        "THE": ["felipe@bora.com.br"],
        "BEL": ["felipe@bora.com.br"],
        "VDC": [
            "jose.sousa@bora.com.br",
            "fernando.sousa@bora.com.br",
            "brandao.alan@bora.com.br",
        ],
        "REC": [
            "cinthya.souza@bora.com.br",
            "patricia.santos@bora.com.br",
            "edvanio.silva@bora.com.br",
    ...
    for k, v in gachoices:
        ...
        if not pdr.empty:
            send = [
                "gabriel.torres@bora.com.br",
                "alan@bora.com.br",
                "gabriel.moura@bora.com.br",
                "thiago@bora.com.br",
                "rafael.rocha@bora.com.br",
            ]
            for i in result:
                send.append(i)
            print(send)
            ...
            if k in ("6", "7"):
                ...
                try:
                    ...
                    if not row.empty:
                        ...
                        print(row)
                        row["saida_veic"] = pd.to_datetime(
                            row["saida_veic"], format="%d/%m/%Y"
                        ).dt.strftime("%d/%m/%Y")
                        row["leadtime"] = pd.to_datetime(
                            row["leatime"], format="%d/%m/%Y"
                        ).dt.strftime("%d/%m/%Y")
                        ...
                        try:
                            ...
                            sm.sendmail(
                                get_secret("KH_MDFUSER"), send2, msg.as_string()
                            )
                        except Exception as e:
                            raise e
            ...
            buffer = io.BytesIO()
            pd.ExcelWriter(buffer)
            pdr["saida_veic"] = pd.to_datetime(
                pdr["saida_veic"], format="%d/%m/%Y"
            ).dt.strftime("%d/%m/%Y")
            pdr["leadtime"] = pd.to_datetime(
                pdr["leadtime"], format="%d/%m/%Y"
            ).dt.strftime("%d/%m/%Y")
            ...
            try:
                ...
                return HttpResponse(
                    f"<h3> Falha no envio do email {k} para {send},</h3><p>conteúdo: {msg.as_string()}</p><br><h3>Erro: {e} <br> Tipo: {type(e)}</h3>"
                )
                raise e
...
def bipagemdocrel(request):
    ...
    if request.method == "POST":
        ...
        if date1 and date2 and fil and fil != "Selecione...":
            ...
            query = EtiquetasDocumento.objects.filter(
                pub_date__lte=date2, pub_date__gte=date1, garagem=fil
            ).annotate(
                total=Count("bipagemetiqueta"),
                user=F("bipagemetiqueta__autor__username"),
            ...
            if query:
                for q in query:
                    array.append(
                        {
                            "NR_DOC": q.nr_doc,
                            "GARAGEM": q.get_garagem_display(),
                            "TP_DOC": q.get_tp_doc_display(),
                            "VOLUME": q.volume,
                            "NOTA": q.nota,
                            "USER": q.user,
                            "DATA BIPAGEM": q.pub,
                ...
                response[
                    "Content-Disposition"
                ] = f'attachment; filename="{date1}-{date2}.xlsx"'
                writer = pd.ExcelWriter(response, engine="xlsxwriter")
                df.to_excel(writer, "sheet1", index=False)
                writer.save()
                return response
...
def bipagempalrel(request):
    ...
    if request.method == "POST":
        ...
        if date1 and date2:
            ...
            query = BipagemPalete.objects.filter(
                etq_ref__pub_date__lte=date2, etq_ref__pub_date__gte=date1
            ).annotate(total=Count("cod_barras"))
            if query:
                for q in query:
                    array.append(
                        {
                            "cliente": q.etq_ref.cliente,
                            "codigo": q.cod_barras,
                            "filial": q.get_filial_display(),
                            "data_bipagem": q.bip_date,
                            "volume": q.volume_conf,
                            "nf": q.etq_ref.nota_fiscal,
                            "localizacao": q.etq_ref.localizacao,
                            "autor": q.autor.username,
                        }
                    )
                df = pd.DataFrame(array)
...
def justificativa(request):
    ...
    if request.method == "GET":
        ...
        if date1 and date2 and garagem:
            form = JustificativaEntrega.objects.filter(
                filial__id_garagem=garagem,
                data_emissao__lte=date2,
                data_emissao__gte=date1,
                confirmado=False,
            ).order_by("lead_time")
            ...
            return render(
                request,
                "portaria/etc/justificativa.html",
                {
    ...
    return render(
        request,
        "portaria/etc/justificativa.html",
        {"filiais": filiais, "today": today, "yesterday": yesterday},
    )
...
def confirmjust(request):
    filiais = Filiais.objects.all()
    form = JustificativaEntrega.objects.filter(
        cod_just__isnull=False, desc_just__isnull=False, confirmado=False
    )
    if request.method == "GET":
        date1 = request.GET.get("data1")
        date2 = request.GET.get("data2")
        ...
        if date1 and date2 and filial:
            form = JustificativaEntrega.objects.filter(
                ...
                data_emissao__gte=date1,
                confirmado=False,
                cod_just__isnull=False,
                desc_just__isnull=False,
            )
            return render(
                request,
                "portaria/etc/confirmjustificativas.html",
...
WHERE
    ...
    F4.NOTA_FISCAL = '{nf}'
            ...
            for just in justificativas:
                if (
                    just["carga_encomenda"] == "CARGA DIRETA"
                    or just["carga_encomenda"] == "RODOVIARIO"
                ):
                    cur.execute(
                        f"""
SELECT DISTINCT
...
ORDER BY
    TO_CHAR(A1.DATA_OCORRENCIA, 'DD-MM-YYYY')
                    ...
                    just["romaneios"] = [
                        ...
                        if just["conhecimento"] == rom["conhecimento"]
                    ]  # and just['garagem'] == rom['garagem']]
...
def pivot_rel_just(date1, date2):
    ...
    if date1 and date2:
        qs = JustificativaEntrega.objects.filter(
            data_emissao__lte=date2, data_emissao__gte=date1
        )
    else:
        ...
        array.append(
            {
                ...
                "EM_ABERTO": q.em_aberto,
                "LOCAL_ENTREGA": q.local_entreg,
                "NOTA_FISCAL": q.nota_fiscal,
                "COD_JUST": q.cod_just,
                "DESC_JUST": q.desc_just,
                "AUTOR": q.autor,
                "DATA_ENTREGA": "NAO ENTREGUE"
                if q.data_entrega == compare_date.date()
                else q.data_entrega,
            }
        )
    ...
    response[
        "Content-Disposition"
    ] = f"attachment; filename=relatorio-justificativa{datetime.date.today()}.xlsx"
    writer = pd.ExcelWriter(response, engine="xlsxwriter")
...
async def get_xmls_api(request):
    ...
    try:
        ...
        print(f"Error: {e}, error_type: {type(e).__name__}")
        return HttpResponse(
            f"<h2> Ocorreu um erro durante a consulta - XML </h2> <br> Erro: {e} <br> Tipo do erro: {type(e).__name__}"
        )
...
def compras_index(request):
    filiais = Filiais.objects.all()
    item_solicitante = (
        SolicitacoesCompras.objects.exclude(
            Q(status="CONCLUIDO") | Q(status="CANCELADO")
        )
        .filter(
    ...
    metrics = SolicitacoesCompras.objects.annotate(
        ...
        andam=Count(
            "id",
            filter=(Q(status="ANDAMENTO") | Q(status="APROVADO") | Q(status="ABERTO")),
        ),
    ).aggregate(concluido=Sum("concl"), andamento=Sum("andam"))
    return render(
        request,
        "portaria/etc/compras.html",
        {"filiais": filiais, "item_solicitante": item_solicitante, "metrics": metrics},
    )
        ...
        if idsolic:
            try:
                ...
                obj = SolicitacoesCompras.objects.create(
                    ...
                    autor=request.user,
                    anexo=anexo,
                    solicitante=username or None,
                    email_solic=email or None,
                )
                obj.save()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            prod = ProdutosSolicitacoes.objects.create(
                produto=p_item, qnt_itens=int(p_qty), solic_ref=obj
            )

        except:
            messages.error(
                request, f"Erro ao criar a Solicitação, por favor verifique os dados"
            )
    return render(
        request, "portaria/etc/lancardireto.html", {"filiais": keyga, "deps": deps}
    )
...
def painel_compras(request):
    ...
    form = (
        SolicitacoesCompras.objects.all()
        .exclude(Q(status="CONCLUIDO") | Q(status="CANCELADO"))
        .order_by("data")
    )
    ...
    if request.method == "GET":
        ...
        if filter:
            try:
                ...
                form = SolicitacoesCompras.objects.filter(
...
def painel_compras_concluido(request):
    if request.method == "POST":
        ...
        print(form)
        return render(
            request, "portaria/etc/painelcomprasconcluido.html", {"form": form}
        )
    else:
        print("aaaa")
...
def edit_compras(request, id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if forma_pgt != "":
                obj.forma_pgt = forma_pgt
            if responsavel != "":
                obj.responsavel_id = responsavel
            if categoria != "":
                obj.categoria = categoria
            if get_anexo != "" and bool(get_anexo) is True:
                obj.anexo = get_anexo
            if dt_venc != "" and dt_venc is not None:
                obj.dt_vencimento = dt_venc
            if prazo != "" and prazo is not None:
                obj.prazo_conclusao = prazo
            if obs != "" and obs is not None:
                obj.obs = obs
            if textarea and textarea != "<p><br></p>":
                insert_entradas_cpr(request, obj, textarea, files)
    ...
    return render(
        request,
        "portaria/etc/edit_compras.html",
        {
            "obj": obj,
            "filchoices": filchoices,
            "stschoices": stschoices,
            "dpchoices": dpchoices,
            "rpchoices": rpchoices,
...
def insert_entradas_cpr(request, obj, textarea, files):
    if obj:
        try:
            entrada = {"obs": textarea, "cpr_ref": obj, "ultima_att": request.user}
            if len(files) > 0:
...
def sugestoesedenuncias(request):
    if request.method == "POST":
        ...
        if title and texto and categoria:
            obj = Sugestoes.objects.create(
                titulo=title, texto=texto, categoria=categoria
            )
...
def estoque_index(request):
    ...
    return render(
        request,
        "portaria/estoque/estoqueindex.html",
        {"solics": solic, "metrics": metrics, "itens_solicitados": itens_solicitados},
    )
...
def estoque_nova_solic(request):
    ...
    filiais = Filiais.objects.all()
    funcionarios_clt = Funcionarios.objects.all().order_by("nome").values("nome")
    funcionarios_pj = FuncPj.objects.all().order_by("nome").values("nome")

    funcionarios = list(funcionarios_clt) + list(funcionarios_pj)

    if request.method == "POST":
        data = {"autor": request.user, "data_solic": timezone.now()}
    ...
    if request.method == "POST":
        if request.POST.get("type") == "GROUP":
            ...
            if group_name != "":
                ...
                messages.success(
                    request, f"grupo {group_name} adicionado com sucesso! "
                )
            else:
...
def estoque_detalhe(request, id):
    ...
    if request.method == "POST":
        ...
        if area and area != "<p><br></p>" and e_to:
            ...
            if item.funcionario_clt:
                msg["Subject"] = f"Solicitação de EPI - {item.funcionario_clt}"
            else:
                msg["Subject"] = f"Solicitação de EPI - {item.funcionario_pj}"
            msg["From"] = "bora@bora.com.br"
            msg["To"] = e_to
            ...
            if myfile is not None:
                for q in myfile:
                    ...
                    msg.attach(part)
            msg.attach(MIMEText(area, "html"))

            user = get_secret("E_UMAIL")
            passw = get_secret("E_UPASS")
            try:
                sm = smtplib.SMTP(get_secret("E_HOST"), get_secret("E_PORT"))
                sm.ehlo()
                sm.starttls()
                sm.login(user, passw)

                sm.sendmail("bora@bora.com.br", e_to, msg.as_string().encode("utf-8"))
                sm.quit()
            ...
            messages.error(
                request, "Algo deu errado, gentileza verifique os parâmetros."
            )
        ...
        if desc and qnt and tipo and tam:
            try:
                ...
                EstoqueItens.objects.create(
                    desc=desc, tamanho=tam, quantidade=qnt, tipo=tipo
                )
                messages.success(request, "Item cadastrado com sucesso !!")
            except Exception as e:
                messages.error(
                    request, "Algo deu errado, por gentileza verifique os parâmetros."
                )
                print(e)
            else:
...
def get_funcionarios_demissao(request):
    ...
    for q in res:
        try:
            ...
            obj = Demissoes.objects.create(
                empresa=q["codigoempresa"],
                filial=q["codigofl"],
                nome=q["nomefunc"],
                cpf=q["nrdocto"],
                dtadmissao=q["dtadmfunc"],
            )
        except Exception as e:
            print(f"Error:{e}, error_type:{type(e).__name__}")
