...
class TPaletesForm(forms.Form):
    origem_ = forms.ChoiceField(choices=GARAGEM_CHOICES, required=True)
    destino_ = forms.ChoiceField(choices=GARAGEM_CHOICES, required=True)
...
class ChecklistForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "obs_cavalo": Textarea(
                attrs={"cols": 30, "rows": 3, "style": "resize:none;"}
            ),
            "obs_carreta": Textarea(
                attrs={"cols": 30, "rows": 3, "style": "resize:none;"}
            ),
        }
...
class SolicEstoque(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for index in range(int(extra_fields)):
            self.fields["item_{index}".format(index=index)] = forms.CharField()
